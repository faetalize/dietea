 ...
 * Return a snapshot so outside consumers cannot mutate component state, and
 * include the calculated goals used by the live AI read tool.
 */
export function getTrackerState() {
  return {
    ...trackerState,
    completed: { ...trackerState.completed },
    goals: getGoals()
  };
}
...
function getDisplaySupplements() {
  return getSupplementCatalog(getGoals().proteinGoalG);
}
